...
const AddTask = () => {
    ...
    const taskAddedData = (e)=>{
        const field = e.target.name;
        const value = e.target.value;
        const newData = { ...taskAdded };
        newData[field] = value;
        setTaskAdded(newData)
    } 
    // new task added
    const handleSubmit = (e) => {
        var formdata = new FormData();
        formdata.append("message", taskAdded.message);
        formdata.append("due_date", taskAdded.date);
        formdata.append("priority", taskAdded.priority);
        formdata.append("assigned_to", taskAdded.assign);
        var requestOptions = {
          method: 'POST',
          headers: myHeaders,
          body: formdata,
            ...
            <div className="mx-auto max-w-2xl mt-8">
                <form action="" onSubmit={handleSubmit}>
                    ...
                    <div className="h-auto">
                        ...
                        <div className="relative my-6">
                            <input id="id-l02" type="datetime-local"   name="date" placeholder="your name" onChange={(e)=>taskAddedData(e)} className="relative w-full h-12 px-4 placeholder-transparent transition-all border rounded outline-none focus-visible:outline-none peer border-slate-200 text-slate-500 autofill:bg-white invalid:border-slate-200 focus:border-emerald-500 focus:outline-none disabled:cursor-not-allowed disabled:bg-slate-50 disabled:text-slate-400"
                            />
                        ...
                        <div className="relative my-6">
                            <input id="priority" type="number" name="" placeholder="task priority" onChange={(e)=>taskAddedData(e)} min="1" max="3"  className="relative w-full h-12 px-4 placeholder-transparent transition-all border rounded outline-none focus-visible:outline-none peer border-slate-200 text-slate-500 autofill:bg-white invalid:border-slate-200 focus:border-emerald-500 focus:outline-none disabled:cursor-not-allowed disabled:bg-slate-50 disabled:text-slate-400"
                            />
                        ...
                        <div className="relative my-6">
                            <input id="id-l04" type="number" min="1"  name="assign" placeholder="your assign" onChange={(e)=>taskAddedData(e)}  className="relative w-full h-12 px-4 placeholder-transparent transition-all border rounded outline-none focus-visible:outline-none peer border-slate-200 text-slate-500 autofill:bg-white invalid:border-slate-200 focus:border-emerald-500 focus:outline-none disabled:cursor-not-allowed disabled:bg-slate-50 disabled:text-slate-400"
                            />
